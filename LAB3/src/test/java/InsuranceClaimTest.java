...
import nnkhoi.example.InsuranceClaim;
...
public class InsuranceClaimTest {
    ...
    void testConstructorNullClaimID(){
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> new InsuranceClaim(null, 1000.0));
        assertEquals("Claim ID cannot be null or empty", exception.getMessage());
    }

    @Test
    @DisplayName("processClaim throws exception for null input")
    void testProcessClaimNullInput(){
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> claim.processClaim(null));
        assertEquals("Status update in process claim cannot be null or empty", exception.getMessage());
    }
}
